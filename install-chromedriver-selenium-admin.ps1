...
# SeleniumVBA 설치 경로 확인
$LocalPath = Join-Path $env:LOCALAPPDATA "SeleniumVBA"
$ProgramPath = "C:\Program Files\SeleniumVBA"
...
if (Test-Path $LocalPath) {
    ...
    Write-Host "SeleniumVBA이 사용자 로컬 경로에서 발견되었습니다." -ForegroundColor Green
    Write-Host "설치 경로: $DriverPath" -ForegroundColor Cyan
}
elseif (Test-Path $ProgramPath) {
    $DriverPath = $ProgramPath
    Write-Host "SeleniumVBA이 Program Files 경로에서 발견되었습니다." -ForegroundColor Green
    Write-Host "설치 경로: $DriverPath" -ForegroundColor Cyan
}
else {
    Write-Host "SeleniumVBA이 설치되어 있지 않습니다!" -ForegroundColor Red
    Write-Host "SeleniumVBA을 먼저 설치하세요." -ForegroundColor Yellow
    Write-Host "다음 경로 중 하나에 SeleniumVBA을 설치해주세요:" -ForegroundColor Yellow
